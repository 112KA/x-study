...
export class MaterialReplacer {
	...
	replace(o: Object3D) {
		o.traverse((child: Object3D): void => {
			...
			if (mesh.isMesh) {
				const targetReplaceUnit = this.meshReplaceUnitList.find(
					(unit: MeshReplaceUnit) => {
				...
				if (targetReplaceUnit) {
					...
					mesh.material =
						this.cache[originalMaterial.name] ??
						this.createMaterial(originalMaterial as MeshStandardMaterial);
				}
			}
		});
	}
