...
import { LoadingManager, type Renderer, type WebGPURenderer } from "three/webgpu";
import { assertIsDefined } from "x";
import type { TextureAtlas, TexturePackerData } from "x3/textures/texture-atlas.js";
...
export class AssetManager extends EventDispatcher<AssetManagerEventMap> {
	...
	public async load(resources: ResourceItem[], renderer: Renderer): Promise<void> {
		...
		for (const [index, resource] of resources.entries()) {
			...
			if (type === undefined) {
				const { url } = resource;
				loaded = await this.loadSingle(url);
				targetUrl = url;
				...
				loaded = await Promise.all([this.loadSingle(jsonUrl), this.loadSingle(textureUrl)]);
				this.#resolvers.atlas.resolve(resource, loaded, renderer);
			}

			console.info(LOG_PREFIX, "Loaded", { targetUrl, loaded });
