import { Box2, type Shape, Vector2, type Vector3 } from "three";
...
export class ShapeSampler {
	...
	constructor(private _shapes: Shape[]) {
		...
		let currentFrame = 0;
		for (const chunk of this._distributions) {
	...
	sample(targetPosition: Vector2) {
		const cumulativeTotal = this._distribution[this._distribution.length - 1];
		const lineIndex = this._binarySearch(Math.random() * cumulativeTotal);
		...
		_line.at(Math.random(), targetPosition);
